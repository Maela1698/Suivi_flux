@include('CRM.header')
@include('CRM.sidebar')
<title>TransmissionClient</title>
        ...
        <div class="row">
            <div class="card col-12">
                <div class="justify-content-center align-items-center entete">
                    <h3 class="entete mt-3">TRANSMISSION CLIENT </h3>
                </div>
                <form action="{{ route('DEV.rechercheTransmissionClient') }}" method="post" autocomplete="off">
                    @csrf
                    <div class="row">
                        <div class="col-1">
                        ...
                        <div class="col-2">
                            <div class="input-group">
                                <div class="input-group">
                                    <input type="text" id="nomStade" name="nomStade" class="form-control" placeholder="Stade"
                                        value="{{ $nomStade }}">
                        ...
                        <tbody style="cursor: pointer;">
                            @for ($i = 0; $i < count($transmission); $i++)
                                <tr>
                                    ...
                                    <td> {{ \Carbon\Carbon::parse($transmission[$i]->date_envoie)->format('d/m/y') }}
                                    </td>
                                    <td style="cursor: pointer;">{{ $transmission[$i]->theme }}</td>
                                    <td style="cursor: pointer;">{{ $transmission[$i]->nom_style }}</td>
                                    <td style="cursor: pointer;">{{ $transmission[$i]->nom_modele }}</td>
                                    <td style="cursor: pointer;">{{ $transmission[$i]->nomtier }}</td>
                                    <td style="cursor: pointer;">{{ $transmission[$i]->type_saison }}</td>
                                    <td style="cursor: pointer;">{{ $transmission[$i]->type_stade }}</td>
                                    <td style="cursor: pointer;">{{ $transmission[$i]->quantite }}</td>
                                    <td style="cursor: pointer;">{{ $transmission[$i]->lieu_destination }} </td>
                                    <td style="cursor: pointer;">{{ $transmission[$i]->mode_envoie }} </td>
                                    <td style="cursor: pointer;">{{ $transmission[$i]->awb }} </td>
                                    <td style="cursor: pointer;">{{ $transmission[$i]->commentaire }} </td>
                                </tr>
                            @endfor
                        </tbody>
                    </table>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        nomSaison.addEventListener('input', function() {
            ...
            var xhr = new XMLHttpRequest();
            xhr.open('GET', '{{ route('DEV.rechercheStade') }}?type_stade=' + encodeURIComponent(query),
                true);
            xhr.onload = function() {
                if (xhr.status === 200) {
                    var saisons = JSON.parse(xhr.responseText);
