                    ...
                    <div class="card-body d-flex align-items-center justify-content-between" style="height: 100%;">
                        <div>
                            <h3 class="card-title text-white" style="margin-bottom: 5px; font-size: calc(0.1em + 1vw);">
                                Qte contr</h3>
                            <div class="d-inline-block">
                                <h2 class="text-white" style="font-size: calc(0.5em + 1vw);">{{ $qteContr }}</h2>
                            </div>
                        </div>
                    ...
                    <div class="card-body d-flex align-items-center justify-content-between" style="height: 100%;">
                        <div>
                            <h3 class="card-title text-white" style="margin-bottom: 5px; font-size: calc(0.1em + 1vw);">
                                Taux rejet</h3>
                            <div class="d-inline-block">
                                <h2 class="text-white" style="font-size: calc(0.5em + 1vw);">{{ number_format($tauxrejet, 2, '.', ' ') }}</h2>
                            </div>
                        </div>
                    ...
                    <div class="card-body d-flex align-items-center justify-content-between" style="height: 100%;">
                        <div>
                            <h3 class="card-title text-white" style="margin-bottom: 5px; font-size: calc(0.1em + 1vw);">
                                Taux retouche</h3>
                            <div class="d-inline-block">
                                <h2 class="text-white" style="font-size: calc(0.5em + 1vw);">{{ number_format($tauxretouche, 2, '.', ' ') }}</h2>
                            </div>
                        </div>
        ...
        <div class="row">
            <div class="card col-12">
                <div class="justify-content-center align-items-center entete">
                    <h3 class="entete mt-3">CONTROLE FINAL </h3>
                </div>
                <form action="{{ route('DEV.rechercheControleFinal') }}" method="post" autocomplete="off">
                    @csrf
                    <div class="row">
                        <div class="col-1">
                        ...
                        <div class="col-2">
                            <div class="input-group">
                                <input type="text" id="nomEmploye" name="nomEmploye" class="form-control" placeholder="Patronier"
                                    value="{{ $nomEmploye }}">
                        ...
                        <div class="col-1">
                            <div class="input-group">
                               <select class="form-control" name="typeRetouche">
                                <option value="">Retouche</option>
                                @if (!empty($idretouche))
                                <option value="{{ $idretouche }}">{{ $retouche }}</option>
                                @endif
                                @for ($t=0; $t < count($typeRetouche); $t++)
                                <option value="{{ $typeRetouche[$t]->typeretouche }}">{{ $typeRetouche[$t]->typeretouche }}</option>
                                @endfor
                            ...
                            @for ($i = 0; $i < count($final); $i++)
                                <tr>
                                    <td style="cursor: pointer;">
                                        {{ \Carbon\Carbon::parse($final[$i]->datedebut)->format('d/m/y') }}
                                    </td>
                                    <td> {{ \Carbon\Carbon::parse($final[$i]->datefin)->format('d/m/y') }}</td>
                                    <td> {{ \Carbon\Carbon::parse($final[$i]->deadline)->format('d/m/y') }}
                                    </td>
                                    <td style="cursor: pointer;">{{ $final[$i]->theme }}</td>
                                    <td style="cursor: pointer;">{{ $final[$i]->nom_style }}</td>
                                    <td style="cursor: pointer;">{{ $final[$i]->nom_modele }}</td>
                                    <td style="cursor: pointer;">{{ $final[$i]->nomtier }}</td>
                                    <td style="cursor: pointer;">{{ $final[$i]->type_saison }}</td>
                                    <td style="cursor: pointer;">{{ $final[$i]->stadesdc }}</td>
                                    @if ( $final[$i]->retouche==1 )
                                    <td style="cursor: pointer;">Oui</td>
                                    @else
                                    <td style="cursor: pointer;">Nom</td>
                                    @endif
                                    <td style="cursor: pointer;">{{ $final[$i]->typeretouche }}</td>
                                    <td style="cursor: pointer;">{{ $final[$i]->qtecontrole }}</td>
                                    <td style="cursor: pointer;">{{ $final[$i]->qteretouche }} </td>
                                    <td style="cursor: pointer;">{{ $final[$i]->qterejet }} </td>
                                    <td style="cursor: pointer;">{{ number_format($final[$i]->tauxretouche, 2, '.', ' ') }} %</td>
                                    <td style="cursor: pointer;">{{ number_format($final[$i]->tauxrejet, 2, '.', ' ') }} %</td>
                                    <td style="cursor: pointer;">{{ $final[$i]->commentaire }} </td>
                                </tr>
                            @endfor
                        </tbody>
                    </table>
