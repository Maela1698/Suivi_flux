                    ...
                    <div class="card-body d-flex align-items-center justify-content-between" style="height: 100%;">
                        <div>
                            <h3 class="card-title text-white" style="margin-bottom: 5px; font-size: calc(0.1em + 1vw);">
                                Nbr Occurence</h3>
                            <div class="d-inline-block">
                                <h2 class="text-white" style="font-size: calc(0.5em + 1vw);">{{ $nbrOcc }}</h2>
                            </div>
                        </div>
                    ...
                    <div class="card-body d-flex align-items-center justify-content-between" style="height: 100%;">
                        <div>
                            <h3 class="card-title text-white" style="margin-bottom: 5px; font-size: calc(0.1em + 1vw);">
                                % Occurence</h3>
                            <div class="d-inline-block">
                                <h2 class="text-white" style="font-size: calc(0.5em + 1vw);">{{ number_format($pourcentage, 2, '.', ' ') }}</h2>
                            </div>
                        </div>
        ...
        <div class="row">
            <div class="card col-12">
                <div class="justify-content-center align-items-center entete">
                    <h3 class="entete mt-3">CONTROLE PATRONAGE </h3>
                </div>
                <form action="{{ route('DEV.rechercheControlePatronage') }}" method="post" autocomplete="off">
                    @csrf
                    <div class="row">
                        <div class="col-1">
                        ...
                        <div class="col-1">
                            <div class="input-group">
                               <select class="form-control" name="typeOccurence">
                                <option value="">Occurence</option>
                                @if (!empty($idOccurence))
                                <option value="{{ $idOccurence }}">{{ $occurence }}</option>
                                @endif
                                @for ($t=0; $t < count($typeOccurence); $t++)
                                <option value="{{ $typeOccurence[$t]->occurence }}">{{ $typeOccurence[$t]->occurence }}</option>
                                @endfor
                    ...
                    <table class="table student-data-table m-t-20 table-hover mt-3" style="color: black">
                        <thead>
                            <tr>
                                ...
                                <th>Occurence</th>
                                <th>Type occurence</th>
                                <th>Commentaire</th>
                            </tr>
                        </thead>
                        <tbody style="cursor: pointer;">
                            @for ($i = 0; $i < count($contrPatronage); $i++)
                                <tr>
                                    <td style="cursor: pointer;">{{ $contrPatronage[$i]->nom_patronier }} {{ $contrPatronage[$i]->prenom_patronier }}</td>
                                    <td style="cursor: pointer;">
                                        {{ \Carbon\Carbon::parse($contrPatronage[$i]->datedebut)->format('d/m/y') }}
                                    </td>
                                    <td> {{ \Carbon\Carbon::parse($contrPatronage[$i]->datefin)->format('d/m/y') }}</td>
                                    <td> {{ \Carbon\Carbon::parse($contrPatronage[$i]->deadline)->format('d/m/y') }}
                                    </td>
                                    <td style="cursor: pointer;">{{ $contrPatronage[$i]->theme }}</td>
                                    <td style="cursor: pointer;">{{ $contrPatronage[$i]->nom_style }}</td>
                                    <td style="cursor: pointer;">{{ $contrPatronage[$i]->nom_modele }}</td>
                                    <td style="cursor: pointer;">{{ $contrPatronage[$i]->nomtier }}</td>
                                    <td style="cursor: pointer;">{{ $contrPatronage[$i]->type_saison }}</td>
                                    <td style="cursor: pointer;">{{ $contrPatronage[$i]->stadesdc }}</td>
                                    <td style="cursor: pointer;">{{ $contrPatronage[$i]->quantitesdc }} pcs</td>
                                    @if ($contrPatronage[$i]->occurence == 1)
                                        <td style="cursor: pointer;">Oui</td>
                                    @elseif ($contrPatronage[$i]->occurence == 0)
                                        <td style="cursor: pointer;">Non</td>
                                    @endif

                                    <td style="cursor: pointer;">{{ $contrPatronage[$i]->type_occurence }} </td>
                                    <td style="cursor: pointer;">{{ $contrPatronage[$i]->commentaire }} </td>
                                </tr>
                            @endfor
                        </tbody>
                    </table>
