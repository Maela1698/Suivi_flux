        ...
        <div class="row">
            <div class="card col-12">
                <div class="justify-content-center align-items-center entete">
                    <h3 class="entete mt-3">RAPPORT FINITION </h3>
                </div>

                <form action="{{ route('DEV.rechercheRapportFinition') }}" method="post" autocomplete="off">
                    @csrf
                    <div class="row">
                        <div class="col-1">
                ...
                <div class="table-responsive" style="margin-top: -15px;">
                    <table class="table student-data-table m-t-20 table-hover mt-3" style="color: black">
                        ...
                        <tbody style="cursor: pointer;">
                            @for ($i = 0; $i < count($finition); $i++)
                                <tr>
                                    <td style="cursor: pointer;">
                                        {{ \Carbon\Carbon::parse($finition[$i]->datedebut)->format('d/m/y') }}
                                    </td>
                                    <td> {{ \Carbon\Carbon::parse($finition[$i]->datefin)->format('d/m/y') }}</td>
                                    <td> {{ \Carbon\Carbon::parse($finition[$i]->deadline)->format('d/m/y') }}
                                    </td>
                                    <td style="cursor: pointer;">{{ $finition[$i]->theme }}</td>
                                    <td style="cursor: pointer;">{{ $finition[$i]->nom_style }}</td>
                                    <td style="cursor: pointer;">{{ $finition[$i]->nom_modele }}</td>
                                    <td style="cursor: pointer;">{{ $finition[$i]->nomtier }}</td>
                                    <td style="cursor: pointer;">{{ $finition[$i]->type_saison }}</td>
                                    <td style="cursor: pointer;">{{ $finition[$i]->stadesdc }}</td>
                                    <td style="cursor: pointer;">{{ $finition[$i]->nom }} {{ $finition[$i]->prenom }}</td>
                                    <td style="cursor: pointer;">{{ $finition[$i]->qtefini }}</td>
                                    <td style="cursor: pointer;">{{ $finition[$i]->minuteproduite }} </td>
                                    <td style="cursor: pointer;">{{ $finition[$i]->minutepresence }} </td>
                                    <td style="cursor: pointer;">{{ number_format($finition[$i]->efficiencedev, 3, '.', ' ') }} </td>
                                    <td style="cursor: pointer;">{{ $finition[$i]->commentaire }} </td>
                                </tr>
                            @endfor
                        </tbody>
                    </table>
