        ...
        <div class="row">
            <div class="card col-12">
                ...
                <div class="justify-content-center align-items-center entete">
                    <h3 class="entete mt-3">LISTE DES DEMANDES</h3>
                </div>

                <form action="{{ route('DEV.rechercheAccueil') }}" method="post" autocomplete="off">
                    @csrf
                    <div class="row">
                        <div class="col-1">
                    ...
                    <table class="table student-data-table m-t-20 table-hover mt-3" style="color: black">
                        ...
                        <tbody style="cursor: pointer;">
                            @for ($i = 0; $i < count($demande); $i++)
                                <tr>
                                    <td onclick="window.location.href = '{{ route('DEV.detailDemandeClient', ['idDemande' => $demande[$i]->id_demande_client]) }}';"
                                        style="cursor: pointer;">
                                        {{ \Carbon\Carbon::parse($demande[$i]->date_entree_demande)->format('d/m/y') }}</td>
                                    <td onclick="window.location.href = '{{ route('DEV.detailDemandeClient', ['idDemande' => $demande[$i]->id_demande_client]) }}';"
                                        style="cursor: pointer;">
                                        {{ $demande[$i]->theme }}</td>
                                    <td onclick="window.location.href = '{{ route('DEV.detailDemandeClient', ['idDemande' => $demande[$i]->id_demande_client]) }}';"
                                    ...
                                    <td onclick="window.location.href = '{{ route('DEV.detailDemandeClient', ['idDemande' => $demande[$i]->id_demande_client]) }}';"
                                            style="cursor: pointer;">{{ $demande[$i]->nom_style }}</td>
                                    <td> {{ \Carbon\Carbon::parse($demande[$i]->date_livraison)->format('d/m/y') }}</td>
                                    <td onclick="window.location.href = '{{ route('DEV.detailDemandeClient', ['idDemande' => $demande[$i]->id_demande_client]) }}';"
                                        style="cursor: pointer;">{{ $demande[$i]->stadesdc }}</td>
                                    <td onclick="window.location.href = '{{ route('DEV.detailDemandeClient', ['idDemande' => $demande[$i]->id_demande_client]) }}';"
                                        style="cursor: pointer;">{{ $demande[$i]->etapedev }}</td>
                                    <td onclick="window.location.href = '{{ route('DEV.detailDemandeClient', ['idDemande' => $demande[$i]->id_demande_client]) }}';"
                                        style="cursor: pointer;">{{ $demande[$i]->nom_patronier }} {{ $demande[$i]->prenom_patronier }}</td>
                                </tr>
                            @endfor
