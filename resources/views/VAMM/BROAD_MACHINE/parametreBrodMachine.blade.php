...
<div class="col-md-1 fixed-top-right">
    <div class="d-flex flex-column align-items-end">
        ...
        <div class="card p-2 custom-card" id="settings-menu">
            ...
            <form action="{{ route('BRODMACHINE.detailDemandeBroderieMachine') }}" method="get">
                @if (session('idDemandeBrodMachine'))
                    <input type="hidden" name="idDemande" value="{{ session('idDemandeBrodMachine') }}">
                @endif
            ...
            <!-- Bouton Matière Première -->
            <form action="{{ route('BRODMACHINE.listeNombrePointsBrodMachine') }}" method="get">
                <button type="submit" class="btn btn-light mb-2" data-toggle="tooltip" title="Matière Première">
                    <i class="fas fa-box-open"></i>NombrePoints
                </button>
            </form>

            <!-- Bouton SDC -->
            <form action="{{ route('BRODMACHINE.listeConsommableBrodMachine') }}" method="get">
                <button type="submit" class="btn btn-light mb-2" data-toggle="tooltip" title="Matiere premiere">
                    <i class="fas fa-tasks"></i> Consommable
                </button>
            </form>
