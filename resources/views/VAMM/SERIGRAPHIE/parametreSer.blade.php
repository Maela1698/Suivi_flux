...
<div class="col-md-1 fixed-top-right">
    <div class="d-flex flex-column align-items-end">
        ...
        <div class="card p-2 custom-card" id="settings-menu">
            <!-- Detail -->
            <form action="{{ route('SERIGRAPHIE.detailDemandeSerigraphie') }}" method="get">
                @if (session('idDemandeSer'))
                    <input type="hidden" name="idDemandeSer" value="{{ session('idDemandeSer') }}">
                @endif
            ...
            <form action="{{ route('SERIGRAPHIE.listeParametreSer') }}" method="get">
                <button type="submit" class="btn btn-light mb-2" data-toggle="tooltip" title="Matière Première">
                    <i class="fas fa-box-open"></i>Paramètre
                </button>
            </form>
