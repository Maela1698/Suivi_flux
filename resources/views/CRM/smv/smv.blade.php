            ...
            <div class="card-body">
                ...
                <div class="sdc" style="display: flex; align-items: center; justify-content: space-between;">
                    <div class="button-group">
                        <form action="{{ route('CRM.smvApercue') }}" method="get">
                            <button type="submit" class="btn btn-info">Aperçue</button>
                        </form>
                        <form action="{{ route('CRM.ajoutSmv') }}" method="get">
                            <input type="hidden" name="stade" value="{{ $demande[0]->id_stade }}">
                            <button type="submit" class="btn btn-secondary">Ajouter</button>
                        </form>
                        <form action='{{ route('CRM.updateSmv') }}' method='POST'>
                            @csrf
                            <input type="hidden" name="idDemandeClient"
                                value="<%= listev_demandeClient.get(0).getId()%>">
