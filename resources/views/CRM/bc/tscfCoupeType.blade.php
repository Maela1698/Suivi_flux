        ...
        <div class="col-lg-12">
            <div class="card" style="border-radius: 10px;width: 105%;margin-left: -31.5px;">
                <div class="card-header text-center" style="display: flex; justify-content: center;">
                    <h3 class="entete">TABLEAU DE SUIVIS COUPE TYPE</h3>
                </div>
                <form action="{{ route('CRM.tscfCoupeType') }}" method="get">
                    @csrf
                    ...
                    <table class="table table-striped">
                        ...
                        @foreach ($donne as $d)
                            <tr style="color: rgb(77, 77, 77)"
                                onclick="window.location.href = '{{ route('CRM.detailBc', ['idBc' => $d->id_donne_bc]) }}';">
                                <td>
                                    <div class="code">
                                ...
                                <td>{{ $d->numerobc }}</td>
                                <td>{{ $d->fournisseur }}</td>
                                <td>{{ $d->designation }}/{{ $d->ref_tissus }}/{{ $d->composition_tissus }}</td>
                                <td>{{ $d->laize }}</td>
                                <td>{{ $d->couleur }}</td>
                                <td>{{ $d->quantite }}</td>
                                <td>{{ $d->prix_total }}</td>
