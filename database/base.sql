...
alter table listeEmploye
add foreign key (idFonction) references fonctionEmploye (id);

alter table listeEmploye
add foreign key (idSection) references section (id);

alter table listeEmploye
add foreign key (idClassification) references classification (id);
...
create table macrocharge(
    ...
    annee int,
    jourOuvrable int,
    effectif_macro int default 422,
    absence double precision default 0.036,
    heureTravail int default 8,
    efficience_macro double precision default 55,
    heureSup int default 0,
    etat int default 0
);
ALTER TABLE public.macrocharge
ALTER COLUMN absence TYPE double precision
USING absence::double precision;
...
-- END MACRO
ALTER TABLE dataprint ADD COLUMN heuresup double precision; ALTER TABLE dataBrodMain ADD COLUMN heuresup double precision;
